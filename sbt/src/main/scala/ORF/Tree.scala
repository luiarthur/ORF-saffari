  ...
  private def pretty(spacing: Int = 3): Vector[String] = {
    ...
    def paste(l: Vector[String], r: Vector[String]) = {
      def elongate(vs: Vector[String]) = {
        val maxCol = vs.map(_.length).max
        vs.map( s => s + rep(maxCol - s.length) )
      }
    ...
    val top = rep(posL) + rep(spacing+ls(0).length-posL,"_") + ps + rep(spacing+posR,"_") + rep(rs(0).length-posR)
    val bottom = List(ls, Vector(rep(spacing*2 + ps.length)), rs).reduce(paste)
    Vector(top) ++ bottom
  }
}
